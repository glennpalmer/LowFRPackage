#' Summarize cumulative effects for all exposures
#'
#' Compute posterior means and credible intervals for the change in expected outcome if each chemical is shifted from -0.5 to 0.5 at all observed times, with all other exposures held constant at 0.
#'
#' @export
...
#'
summarize_cumulative_effects <- function(fit, varnames, lower=0.025, upper=0.975) {
  # initialize effects
  num_effects <- dim(fit$Sigma)[2]
  num_times <- dim(fit$alpha)[2] / num_effects
  ones_mat <- diag(num_effects)
  x1_mat <- matrix(nrow=num_effects, ncol=(num_effects*num_times))
  x2_mat <- matrix(nrow=num_effects, ncol=(num_effects*num_times))
  for (i in 1:num_effects) {
    x1_mat[i,] <- rep(ones_mat[i,], each=num_times) * -0.5
    x2_mat[i,] <- rep(ones_mat[i,], each=num_times) * 0.5
  }
